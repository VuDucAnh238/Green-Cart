...
const Cart = () => {
    const {products, currency, cartItems, getCartAmount, removeFromCart, getCartCount, updateCartItem, navigate, axios, user, setCartItems} = useAppContext()
    ...
    const getUserAddress = async () => {
        try {
            const {data} = await axios.get('/user/address/get');
            if (data.success) {
    ...
    return products.length > 0 && cartItems ? (
        <div className="flex flex-col md:flex-row mt-16">
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 bg-green-50/60 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <select onChange={(e) => setPaymentOption(e.target.value)} className="w-full border border-gray-300 bg-white px-3 py-2 mt-2 outline-none rounded-full">
                ...
                <button onClick={placeOrder} className="w-full py-3 mt-6 cursor-pointer bg-primary text-white font-medium hover:bg-primary-dull transition rounded-full">
